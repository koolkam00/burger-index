...
import { menuKey } from "@/lib/menus";
import { absoluteUrl } from "@/lib/metadata";
import { BEST_VALUE_PATH, hasPeoplesPriceSentence } from "@/lib/peoples-price";
import { getBestValue, getSnapshotAsOf, getSnapshotFigures } from "@/lib/peoples-price-data";
 ...
 * once it exists, the best value burgers (never before: lib/peoples-price bestValue). Pages whose static
 * HTML changes with the daily People's Price snapshot carry its date when it is the later one: the People's
 * Price pages, /best-burgers (every row's People's Price) and each restaurant page whose menu has the
 * People's Price sentence (3+ answers).
 */
export default function sitemap(): MetadataRoute.Sitemap {
  ...
  return [
    ...
    ...getPricedRestaurants().map((r) => ({
      url: absoluteUrl(`/restaurants/${r.id}`),
      lastModified: hasPeoplesPriceSentence(getSnapshotFigures(menuKey(r))) ? crowd : lastModified,
