...
- **The People's Price page** (`/peoples-price`, `components/worth/PeoplesPriceBoard.tsx`):
  ...
  - **Meta line:** a "Live" badge while the realtime channel is up: the badge shape (24px tall, 6px radius) with an `--ok-bg` fill, `--ink` text at weight 600, a transparent border and an 8px static `--ok-icon` dot, never pulsing.
...
| Every burger (`/burgers`) | Ticket "Cast a line" · H1 "Every burger." · lede "548 burgers, from $6 to $75. Search by name, restaurant or neighborhood." (count and the cheapest and priciest prices, computed) |
...
| Footer | "We index prices, not quality. An original seaside-diner homage: not affiliated with any restaurant, TV show or network." · "Map data © OpenStreetMap contributors, tiles by OpenFreeMap." · "Download the prices (CSV)" (a `deck-link` in the footer nav, where the old JSON download sat) |
