...
- **Hand-check note ("the cook's correction slip"):** the pipeline's hand check (`pipeline/corrections.py`, parsed by `lib/hand-checks.ts`) is shown as its own card under the burger block's links, because it changes the price the page shows. It is a label only: it says that a hand check happened and when, never what was changed or why.
  ...
  - A **withheld** price leaves the restaurant unpriced, so it has no page and no slip; in practice only the "corrected" slip shows. The scrape's own note (`status_detail`) is never shown.
